...
def test_invalid_token(reg_user, create_channel, AccessError):
    ...
    message_edit_request = message_edit_v2("invalid_token", message_id, message)
    assert message_edit_request.status_code == AccessError

def test_non_string_message(reg_user, create_channel, InputError):
    ...
    message_edit_request = message_edit_v2(user_token, message_id, 123923)
    assert message_edit_request.status_code == InputError
        
#  Length of message is over 1000 characters
def test_too_long(reg_user, create_channel, InputError):
    ...
    message_edit_request = message_edit_v2(user_token, message_id, message)
    assert message_edit_request.status_code == InputError

#  message_id refers to a deleted message
def test_delected_message(reg_user, create_channel, InputError):
...
#         Message with message_id was sent by the authorised user making this request
#         The authorised user is an owner of this channel or the **Dreams**
def test_not_owner(reg_user, create_channel, AccessError):
    ...
    message_edit_request = message_edit_v2(another_token, message_id, message)
    assert message_edit_request.status_code == AccessError
