...
def test_no_channel(reg_user, create_channel, AccessError):
    ...
    message_unreact_request = message_unreact_v1(another_token, message_id, react_id)
    assert message_unreact_request.status_code == AccessError
...
def test_invaild_message_id_dm(reg_user, InputError):
    ...
    message_unreact_request = message_unreact_v1(user_token, message_id + 1, react_id)
    assert message_unreact_request.status_code == InputError
...
# react_id is not a valid React ID. The only valid react ID the frontend has is 1
def test_invaild_unreact_id(reg_user, create_channel, InputError):
    clear_v2()
    ...
    message_unreact_request = message_unreact_v1(user_token, message_id, 3)
    assert message_unreact_request.status_code == InputError

# Message with ID message_id does not contain an active React with ID react_id from the authorised user
def test_not_react(reg_user, create_channel, InputError):
    clear_v2()
    ...
    message_unreact_request = message_unreact_v1(user_token, message_id, react_id)
    assert message_unreact_request.status_code == InputError

# double unreact
def test_double_unreact(reg_user, create_channel, InputError):
    clear_v2()
...
def test_invalid_token(reg_user, create_channel, AccessError):
    ...
    message_unreact_request = message_unreact_v1("invalid_token", message_id, react_id)
    assert message_unreact_request.status_code == AccessError
