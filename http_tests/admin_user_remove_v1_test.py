...
def test_login_after_removed(reg_user, crt_channel, user_details, InputError):
    clear_v2()
    admin = reg_user(0)
    basic_user = reg_user(1)
    email, pword, *_ = user_details(1)
    admin_user_remove_v1(admin['token'], basic_user['auth_user_id'])
    remove_request = auth_login_v2(email, pword)
    assert remove_request.status_code == InputError

def test_invalid_u_id(reg_user, InputError):
    clear_v2()
    admin = reg_user(0)
    remove_request = admin_user_remove_v1(admin['token'], admin['auth_user_id']+1)
    assert remove_request.status_code == InputError

def test_user_is_only_owner(reg_user, InputError):
    clear_v2()
    admin = reg_user(0)
    remove_request = admin_user_remove_v1(admin['token'], admin['auth_user_id'])
    assert remove_request.status_code == InputError
...
def test_auth_user_is_not_owner(reg_user, AccessError):
    ...
    admin = reg_user(0)
    basic_user = reg_user(1)
    remove_request = admin_user_remove_v1(basic_user['token'], admin['auth_user_id'])
...
def test_invalid_token(reg_user, AccessError):
    ...
    reg_user(0)
    basic_user = reg_user(1)
    remove_request = admin_user_remove_v1("Invalid token", basic_user['auth_user_id'])
    assert remove_request.status_code == AccessError
