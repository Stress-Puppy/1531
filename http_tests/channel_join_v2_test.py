...
def test_non_global_owner_join_private_channel(reg_user, crt_channel, AccessError): # Non-global-owner should be unable to join private channel 
    clear_v2()
    auth_user_token = reg_user(0)['token']
    channel_id = crt_channel(auth_user_token, False)['channel_id']
    joined_user_token = reg_user(1)['token']
    join_request = channel_join_v2(joined_user_token, channel_id)
    assert join_request.status_code == AccessError
...
def test_negative_channel_id(reg_user, crt_channel, InputError): # Give invalid channel id
    ...
    joined_user_token = reg_user(1)['token']
    join_request = channel_join_v2(joined_user_token, -1) # Test negative channel_id (always invalid)
    assert join_request.status_code == InputError
...
def test_non_existent_channel_id(reg_user, crt_channel, InputError): # Give invalid channel id
    ...
    channel_id = crt_channel(auth_user_token, True)['channel_id']
    joined_user_token = reg_user(1)['token']
    join_request = channel_join_v2(joined_user_token, channel_id+1) # Test negative channel_id (always invalid)
    assert join_request.status_code == InputError
...
def test_invalid_token(reg_user, crt_channel, AccessError):
    ...
    channel_id = crt_channel(auth_user_token, True)['channel_id']
    reg_user(1)
    join_request = channel_join_v2("Invalid token", channel_id) # Test invalid token (always raises AccessError)
    assert join_request.status_code == AccessError
