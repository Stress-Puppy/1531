...
# Channel ID is not a valid channel
def test_invaild_channel(reg_user, crt_channel, message_text):
    clear_v2()
    ...
    with pytest.raises(InputError):
        message_sendlater_v1(user_token, channel_id + 1, message, time_sent)
...
# the authorised user has not joined the channel they are trying to post to
def test_sender_non_channel_member(reg_user, crt_channel, message_text):
    clear_v2()
    user_token = reg_user(0)['token']
    channel_id = crt_channel(user_token)['channel_id']
    ...
    with pytest.raises(AccessError):
        message_sendlater_v1(user_token_1, channel_id, message, time_sent)
