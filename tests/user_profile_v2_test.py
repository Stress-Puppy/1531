...
from src.user import user_profile_v2, user_profile_setemail_v1, user_profile_setname_v2
...
def test_invalid_u_id(user_details):
    clear_v2()
    eAddress, pword, fName, lName = user_details(0)
    user = auth_register_v2(eAddress, pword, fName, lName)
    with pytest.raises(InputError):
        user_profile_v2(user['token'], user['auth_user_id']+1)
...
def test_invalid_token(user_details):
    ...
    with pytest.raises(AccessError):
        user_profile_v2("invalid_token", user['auth_user_id'])
